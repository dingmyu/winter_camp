...
def save_imgs(args, e1, e2, decoder, iters):
    ...
    mysave_imgs(args, e1, e2, decoder, iters, test_domA, test_domB)
    mysave_restore_imgs(args, e1, e2, decoder, iters, test_domA, test_domB)
    
def mysave_imgs(args, e1, e2, decoder, iters, test_domA, test_domB):
    exps = []
...
def mysave_restore_imgs(args, e1, e2, decoder, iters, test_domA, test_domB):
    exps = []
    ...
    for i in range(args.num_display): 
            seperate_B = torch.full((1, args.sep * (args.resize // 64) * (args.resize // 64)), 0).cuda()
            common_B = e1(test_domB[i].unsqueeze(0))
            B_encoding = torch.cat([common_B, seperate_B], dim=1)
    ...
    vutils.save_image(exps,
                      '%s/experiments_%06d_r.png' % (args.out, iters),
...
def get_test_imgs(args):
    ...
    domA_test = CustomDataset(os.path.join(args.root, 'testA.txt'), transform=comp_transformA)
    domB_test = CustomDataset(os.path.join(args.root, 'testB.txt'), transform=comp_transformB)
